...
//            throw new TestingException("Hit error during wait", e);
	...
	public static synchronized void sendKeyElement(String xPath, String inputData) throws InterruptedException {
		Thread.sleep(1000);
		WebDriver driver = DriverContext.getDriver();
		driver.findElements(By.xpath(xPath)).stream().filter(WebElement::isDisplayed).findFirst().get().sendKeys(inputData);
	}
	...
	public static synchronized void checkFileDownloaded(String location,String fileName, String downloadBtn) throws InterruptedException {
		String downloadedFile = System.getProperty("user.home") + File.separator + location;
		File directory = new File(downloadedFile);
		...
		WebDriver driver = DriverContext.getDriver();
		System.out.println("click download or export button");
		Thread.sleep(2000);
		WebDriverUtil.clickElement(downloadBtn);
		Thread.sleep(5000);

		if (isFileDownloaded(downloadedFile, fileName)) {
